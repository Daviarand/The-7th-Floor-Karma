...
//     [Range(0f, 1f)]
//     public float chanceToGoStraight = 0.6f; 
...
public class MazeGeneratorPerfectLoop : MonoBehaviour
{
    ...
    public GameObject wallPrefab; 
    public GameObject coinPrefab; 
    // Variabel doorPrefab SUDAH DIHAPUS karena kita pakai manual
    public int seed = 12;

    [Header("Penyesuaian Aset")]
    public float wallYOffset = 0f; 
    public float wallHeight = 4f; 
    ...
    public void GenerateMaze()
    {
        ...
        Debug.Log("Labirin Siap! (Tanpa Auto-Spawn Pintu)");
    }
    ...
    void GenerateLogic(Cell[,] grid, float startPos)
    {
        ...
        if (coinPrefab!=null) {
            int lim = Mathf.Min(totalCoins, locs.Count-1);
            for(int i=0; i<lim; i++) Instantiate(coinPrefab, locs[i], coinPrefab.transform.rotation, transform);
        }
        
        // PINTU TIDAK DI-SPAWN DISINI LAGI.
    }
